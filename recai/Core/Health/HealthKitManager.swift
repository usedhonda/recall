...
final class HealthKitManager {
    ...
    private let sendInterval: TimeInterval = 3600 // 1 hour
    ...
    private func queryAndSend(from start: Date, to end: Date) async {
        ...
        ActivityLogger.shared.log(.health, "Queried \(start.formatted(.dateTime.hour().minute()))–\(end.formatted(.dateTime.hour().minute()))")
