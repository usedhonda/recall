...
final class LocationManager: NSObject {
    ...
    var minSendInterval: TimeInterval = 60 {
        didSet {
    ...
    func startUpdates() {
        ...
        if backgroundEnabled && canUseBackground {
            ...
            if backgroundActivitySession == nil {
                ...
                print("[LocationManager] Started CLBackgroundActivitySession")
            }
        }

        startLiveUpdates()
        ActivityLogger.shared.log(.location, "Location updates started")
    }
    ...
    private func handleLocationUpdate(_ location: CLLocation) async {
        ...
        if isInForeground {
            ...
            lastSentLocation = location
            lastSentTime = Date()
            lastSendResult = .sent(status: 0, received: nil, healthReceived: nil, body: nil)
            resetHeartbeatTimer()
    ...
    func updateSendInterval(_ interval: Int) {
        minSendInterval = max(TimeInterval(interval), 30)
    }
